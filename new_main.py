...
# Future plans: separate movements into functions and call func instead
def fill_gaps(edgeimg):
    width = edgeimg.shape[0]
    height = edgeimg.shape[1]

    for i in xrange(2, height - 2):
        for j in xrange(2, width - 2):
            if edgeimg[i * width + j] != 255:
                continue

            count = 0
            loc = 1
            if edgeimg[(i - 1) * width + j] == 255:
                count += 1
            if edgeimg[(i + 1) * width + j] == 255:
                count += 1
                loc = 2
            if edgeimg[i * width + j - 1] == 255:
                count += 1
                loc = 3
            if edgeimg[i * width + j + 1] == 255:
                count += 1
            ...
            if edgeimg[(i - 1) * width + j - 1] == 255 and edgeimg[(i - 1) * width + j] != 255 and \
                    edgeimg[i * width + j - 1] != 255:
                count += 1
                loc = 5
            if edgeimg[(i - 1) * width + j + 1] == 255 and edgeimg[(i - 1) * width + j] != 255 and \
                    edgeimg[i * width + j + 1] != 255:
            ...
            if loc == 1:
                ...
                # ----------
                # Down
                if edgeimg[(i + 2) * width + j] == 255:
                    edgeimg[(i + 1) * width + j] = 128
                    continue

                # Down-right
                if edgeimg[(i + 2) * width + j + 1] == 255 or edgeimg[(i + 2) * width + j + 2] == 255 or \
                        edgeimg[(i + 1) * width + j + 2] == 255:
    ...
    # Changes 128 to 255 in edgeimg
    np.place(edgeimg, edgeimg == 128, 255)
    return edgeimg
